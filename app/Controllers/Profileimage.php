...
class Profileimage extends BaseController
{
    ...
    public function update()
    {
        ...
        if ($size > 3) {
            ...
            return redirect()->back()
                            ->with('warning', 'File too large (max 3MB)');
        }

        $type = $file->getMimeType();

        if ( ! in_array($type, ['image/png', 'image/jpeg', 'image/heic'])) {

            return redirect()->back()
                             ->with('warning', 'Invalid file format (PNG, JPEG or HEIC only)');
        }
    ...
    public function delete()
    {
        if ($this->request->getMethod() === 'post') {
            ...
            $path = WRITEPATH . 'upload/profile_images/' . $user->profile_image;
